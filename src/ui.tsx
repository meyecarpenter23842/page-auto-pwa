...
export function OverviewScreen({snapshot,pages,online,onSelectPage}:{snapshot:BridgeSnapshot|null;pages:BridgePage[];online:boolean;onSelectPage:(p:BridgePage)=>void}){
  ...
  return <div className="screen-content overview-screen">
    ...
    <section className="dashboard-section"><div className="section-title"><h2>Hoạt động gần đây</h2><span className="section-link">Xem tất cả</span></div><div className="activity-list">
      {recent.map(log=>{const page=byId.get(log.pageTabId),ok=log.result==='success',paused=log.action.toLowerCase().includes('pause'),statusTone=ok?'blue':paused?'orange':'red';return <button className="activity-row" type="button" key={log.id} onClick={()=>page&&onSelectPage(page)} disabled={!page}><span className="activity-avatar-wrap">{page?<Avatar page={page} size="sm"/>:<span className={`round-action round-action--${statusTone}`}><Icon name={ok?'play':paused?'pause':'error'} size={16}/></span>}<span className={`activity-avatar-status activity-avatar-status--${statusTone}`}><Icon name={ok?'play':paused?'pause':'error'} size={10}/></span></span><span className="activity-row__copy"><strong>{page?.name??`Page #${log.pageTabId}`}</strong><small>{ok?`Đã đăng bài vào nhóm: ${log.groupUid||'—'}`:log.errorMessage||(paused?'Tạm dừng theo lệnh người dùng':'Tác vụ cần chú ý')}</small></span><time>{fmtTime(log.timestamp)}</time></button>})}
